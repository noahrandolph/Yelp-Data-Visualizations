...
// Phoenix Area ~1 mi^2 Boxes
var min_long = -112.640025
var max_long = -111.450806
var min_lat = 33.185835
var max_lat = 33.783713

// Compute degrees lat per mile (N-S) and degrees long per mile (E-W)
var ns_deg_per_mi = 1 / 69.01
var ew_deg_per_mi = 1 / (Math.cos(
  (([min_lat + max_lat]) / 2) * (Math.PI / 180)
) * 69.1710411)

// Compute miles NS and EW for selected city area
var miles_ns = (max_lat - min_lat) / ns_deg_per_mi;
var miles_ew = (max_long - min_long) / ew_deg_per_mi;

var delta_long = (max_long - min_long) / miles_ew;
var delta_lat = (max_lat - min_lat) / miles_ns;
...
var long = min_long;
var lat = min_lat;
for (i = 0; i <= Math.round(miles_ew); i++) {
  squares[i] = 0
  for (j = 0; j <= Math.round(miles_ns); j++) {
    bounds = [
      [lat, long],
      [lat + delta_lat, long + delta_long]
    ];
    squares[i][j] = L.rectangle(bounds, {
        color: "#2262CC",
        weight: 0.1,
        opacity: 0.2,
        fillOpacity: 0.05,
...
function colorSomeSquares(category, data) {
  var coords = []
  //console.log(data)
...
function putPointsOnMap(category_list, data) {
  ...
  console.log(MarkerLayerGroup);
}

function clearMap() {
  //var marker_list_length = marker_list.length;
  //for (var i = 0; i <= marker_list_length; i++) {
  //MarkerLayerGroup.removeLayer(marker_list[i]);}

  // var MarkerLayerGroup_list_length = MarkerLayerGroup_list.length;
  // for (var i = 0; i <= MarkerLayerGroup_list_length; i++) {
  //   map.removeLayer(MarkerLayerGroup_list[i])
  //   console.log(MarkerLayerGroup_list[i]);
  // }
  console.log(MarkerLayerGroup)
  map.removeLayer(MarkerLayerGroup)
  foo_list.pop();
